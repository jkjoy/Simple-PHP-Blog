...
function jaguar_sns_icon(string $name): string
{
    $icons = [
        ...
        'qq' => ['', '0 0 24 24', '<path d="M12 2C7.8 2 5.7 5.4 5.7 9.4c0 1.2.2 2.3.5 3.3-.9 1.2-1.4 2.7-.8 3.2.4.4 1.2.2 2-.3.8 1.5 2.4 2.4 4.6 2.4s3.8-.9 4.6-2.4c.8.5 1.6.7 2 .3.6-.5.1-2-.8-3.2.3-1 .5-2.1.5-3.3C18.3 5.4 16.2 2 12 2Zm-2.3 8.2c-.6 0-1-.7-1-1.5s.4-1.5 1-1.5 1 .7 1 1.5-.4 1.5-1 1.5Zm4.6 0c-.6 0-1-.7-1-1.5s.4-1.5 1-1.5 1 .7 1 1.5-.4 1.5-1 1.5ZM8.5 13h7c-.3 2-1.5 3-3.5 3s-3.2-1-3.5-3Z"></path>'],
        'wechat' => ['', '0 0 24 24', '<path d="M9.7 3C5.45 3 2 5.83 2 9.32c0 1.98 1.12 3.74 2.86 4.9l-.72 2.13 2.48-1.24c.97.34 2 .52 3.08.52.3 0 .59-.02.88-.04a5.7 5.7 0 0 1-.18-1.42c0-3.55 3.46-6.42 7.73-6.42.16 0 .31 0 .47.02C17.74 5.03 14.2 3 9.7 3Zm-2.6 4.2a.9.9 0 1 1 0-1.8.9.9 0 0 1 0 1.8Zm5.2 0a.9.9 0 1 1 0-1.8.9.9 0 0 1 0 1.8Zm5.83 1.63c-3.24 0-5.87 2.15-5.87 4.8s2.63 4.8 5.87 4.8c.83 0 1.62-.14 2.34-.4l1.89.94-.55-1.62c1.33-.88 2.19-2.22 2.19-3.72 0-2.65-2.63-4.8-5.87-4.8Zm-1.96 3.18a.69.69 0 1 1 0-1.38.69.69 0 0 1 0 1.38Zm3.92 0a.69.69 0 1 1 0-1.38.69.69 0 0 1 0 1.38Z"></path>'],
        'weibo' => ['', '0 0 24 24', '<path d="M17.6 10.1c-.5-.15-.85-.25-.58-.9.58-1.4.64-2.61.01-3.23-1.18-1.17-4.32.04-7.01 2.69-2.02 1.99-3.19 4.1-3.19 5.92 0 3.48 4.48 5.59 8.86 5.59 5.74 0 9.56-3.33 9.56-5.97 0-1.59-1.35-3.13-3.65-3.86ZM15.7 18.35c-3.48.35-6.49-1.23-6.72-3.53-.23-2.29 2.41-4.43 5.89-4.78 3.48-.35 6.49 1.23 6.72 3.52.23 2.3-2.41 4.44-5.89 4.79Zm-.49-6.31c-1.66-.43-3.54.4-4.19 1.86-.66 1.46.16 3 1.82 3.43 1.66.43 3.54-.4 4.2-1.86.65-1.46-.17-3-1.83-3.43Zm-1.22 3.75c-.57.38-1.34.27-1.72-.25-.37-.52-.21-1.25.36-1.63.57-.38 1.34-.27 1.71.25.38.52.22 1.25-.35 1.63ZM24 7.27a5.2 5.2 0 0 0-7.25-4.8l.72 1.49a3.54 3.54 0 0 1 4.87 4.65L24 9.1c.14-.59.17-1.21 0-1.83Zm-3.23.7a2.62 2.62 0 0 0-3.49-2.68l.73 1.5a.95.95 0 0 1 1.24.92l1.52.26Z"></path>'],
...
function jaguar_render_post_navigation(array $post): string
{
    ...
    <nav class="navigation post-navigation" aria-label="<?= h(sblog_t('文章导航')) ?>">
      <div class="nav-links">
        <?php if ($neighbors['newer']): ?><div class="nav-previous"><a href="<?= h(url_for('post', ['slug' => (string)$neighbors['newer']['slug']])) ?>" rel="prev"><span class="meta-nav"><?= h(sblog_t('上一篇')) ?></span><span class="post-title"><?= h((string)$neighbors['newer']['title']) ?></span></a></div><?php endif; ?>
        <?php if ($neighbors['older']): ?><div class="nav-next"><a href="<?= h(url_for('post', ['slug' => (string)$neighbors['older']['slug']])) ?>" rel="next"><span class="meta-nav"><?= h(sblog_t('下一篇')) ?></span><span class="post-title"><?= h((string)$neighbors['older']['title']) ?></span></a></div><?php endif; ?>
      </div>
    </nav>
    <?php
    return (string)ob_get_clean();
