...
<body class="<?= h($bodyClass) ?>">
  ...
  <div class="page" id="body-wrap">
    ...
    <main class="layout<?= $flash ? ' has-flash' : '' ?>" id="content-inner">
      ...
      <?php if ($isPost): ?>
        <div id="post"><article class="post-content" id="article-container"><?= markdown_to_html((string)$post['content']) ?></article><?php if (is_live_content($post)): ?><div class="post-actions bf-post-actions"><form data-bf-like method="post" action="<?= h(url_for('like_post')) ?>"><?= csrf_field() ?><input type="hidden" name="post_id" value="<?= (int)$post['id'] ?>"><button class="btn-beautify" type="submit" aria-pressed="<?= visitor_liked_post((int)$post['id']) ? 'true' : 'false' ?>"<?= visitor_liked_post((int)$post['id']) ? ' disabled' : '' ?>><i class="ri-heart-line" aria-hidden="true"></i> <?= h(sblog_t('点赞')) ?> <span data-bf-like-count><?= post_like_count((int)$post['id']) ?></span></button></form><span data-bf-like-status role="status"></span></div><?php endif; ?><?php if ($isPost): ?><div class="tag_share"><div class="post-meta__tag-list"><?= render_tag_chips($post) ?></div></div><?php endif; ?><?= render_comments_section($post) ?></div>
