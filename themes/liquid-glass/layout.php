...
$tagline = trim(setting('site_tagline')) ?: trim(setting('home_intro')) ?: '记录技术、灵感与生活片段。';
...
<body class="<?= h($bodyClass) ?> <?= h($viewClass) ?>">
  ...
  <header class="aqua-header">
    <div class="aqua-nav aqua-glass">
      ...
      <nav class="aqua-nav__links" id="aqua-nav-links" aria-label="主导航">
      ...
      <div class="aqua-nav__tools">
        ...
        <button class="aqua-icon-button aqua-menu-toggle" type="button" aria-label="打开菜单" aria-controls="aqua-nav-links" aria-expanded="false">
  ...
  <?php if ($viewClass === 'aqua-view-home'): ?>
    <section class="aqua-hero">
      <div class="aqua-hero__copy aqua-reveal">
        <span class="aqua-kicker">PERSONAL JOURNAL</span>
        <h1><?= h($siteName) ?></h1>
        <p><?= h($tagline) ?></p>
        <div class="aqua-hero__actions">
          <a class="aqua-primary-button" href="#aqua-feed-title">阅读文章 <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14m7-7-7 7-7-7"/></svg></a>
          <a class="aqua-text-button" href="<?= h(url_for('archives')) ?>">查看归档</a>
        </div>
      </div>
      <div class="aqua-hero__material aqua-glass aqua-reveal" style="--reveal-delay:.08s">
        ...
        <div><strong><?= h((string)$postCount) ?></strong><span>篇文章</span></div>
        <i></i>
        <div><strong><?= h((string)$tagCount) ?></strong><span>个标签</span></div>
      </div>
    </section>
  <?php endif; ?>
  ...
  <footer class="aqua-footer">
    <div>
      ...
      <nav aria-label="页脚导航"><a href="<?= h(url_for('rss')) ?>">RSS</a><a href="<?= h(url_for('sitemap')) ?>">Sitemap</a><?php $beian = trim(setting('footer_beian')); ?><?php if ($beian !== ''): ?><a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer"><?= h($beian) ?></a><?php endif; ?></nav>
    </div>
  </footer>
  <?php theme_action('footer_after', $themeContext); ?>

  <button class="aqua-backtop aqua-glass" type="button" aria-label="返回顶部" title="返回顶部">
