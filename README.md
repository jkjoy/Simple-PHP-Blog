...
- 项目内置 `english-language` 插件，启用后会将前台、登录页和后台系统界面翻译为英文，不修改数据库中的文章内容。
...
- 一键更新会保留 `data/`、`cache/`、`uploads/`、用户自建主题和插件，递归合并发布包中的 `themes/` 与 `plugins/`，并将被覆盖的程序、主题与插件文件备份到 `cache/update-backup-*`
